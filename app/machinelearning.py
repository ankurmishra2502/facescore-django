...
face_recognition_model=pickle.load(open(os.path.join(STATIC_DIR,'models/machinelearing_face_person_identity.pkl'),mode='rb'))
emotion_recognition_model=pickle.load(open(os.path.join(STATIC_DIR,'models/machinelearing_face_emotion.pkl'),mode='rb'))
    ...
    if len(detections) > 0:
        for i , confidence in enumerate(detections[0,0,:,2]):
            if confidence > 0.5:
                ...
                cv2.imwrite(os.path.join(settings.MEDIA_ROOT,'output/process.jpg'),image)
                cv2.imwrite(os.path.join(settings.MEDIA_ROOT,'output/roi_{}.jpg'.format(count)),face_roi)
...
# img, results = pipeline_model('./data/obama8.jpg')
